...
const SignUp02Page = () => {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      console.error(error)
      toast.error(
        error.message ?? "Something went wrong when signup. Try again later.",
        {
          position: "top-center",
        }
      )
  ...
  const handleCheckDuplicateEmail = async (email: string) => {
    try {
      ...
      console.error(error)
      toast.error("Something went wrong when signup. Try again later.", {
        position: "top-center",
      })
    }
  }

  const handleCheckDuplicateUserName = async (username: string) => {
    try {
      const { isDuplicate } = await userService.checkDuplicateUsername(username)
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      onBlur={async () => {
                        await handleCheckDuplicateUserName(field.value)
                      }}
                    />
                  </FormControl>
                  <FormMessage />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      onBlur={async () => {
                        await handleCheckDuplicateEmail(field.value)
                      }}
                    />
                  </FormControl>
                  <FormMessage />
